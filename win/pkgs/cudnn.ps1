...
# URL in the form of:
#     https://developer.download.nvidia.com/compute/redist/cudnn/v8.0.5/cudnn-11.1-windows-x64-v8.0.5.39.zip
...
for ($i=8; ($i -ge 8) -and (-not (Test-Path cudnn.zip)); $i--)
{
    for ($j=2; ($j -ge 0) -and (-not (Test-Path cudnn.zip)); $j--)
    {
        ...
        {
            for ($l=26; ($l -ge 0) -and (-not (Test-Path cudnn.zip)); $l--)
            {
                ...
                $cudnn_name=("$cudnn_name" -replace '11.[0-3]','11.4')
