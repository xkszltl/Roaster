...
[ -e $STAGE/openblas ] && ( set -xe
    ...
    until git clone --depth 1 --single-branch -b "$(git ls-remote --tags "$GIT_MIRROR/xianyi/OpenBLAS.git" | sed -n 's/.*[[:space:]]refs\/tags\/\(v[0-9\.]*\)[[:space:]]*$/\1/p' | sort -V | tail -n1)" "$GIT_MIRROR/xianyi/OpenBLAS.git"; do echo 'Retrying'; done
    cd OpenBLAS
